...
$(document).ready(function(){
  ...
  $("#links-list").children().each(function() {
    ...
    var findStatus = $(this).find('.link_read').text().replace(/\s/g, '');
    var readStatus = findStatus.split(":").pop();

    if (readStatus == 'true') {
      $(this).addClass('read')

      var buttonSelector = $(this).find('.mark-read')

      buttonSelector.removeClass('mark-read')
      buttonSelector.addClass('mark-unread')
      buttonSelector.html("Mark Unread")
    }
  });
});
